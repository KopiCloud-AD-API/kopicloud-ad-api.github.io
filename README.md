...
documentation directory. This directory
is named `docs` .

The placement will look something like this:
...
within the `docs` directory are copied by
MkDocs to the built site unaltered. 
...
`README.md`. In that way, when users are browsing your
...
The `nav` configuration setting in the `nav.yaml` file
...
source files for the above configuration would be located at `docs/README.md` and
`docs/about.md`.
